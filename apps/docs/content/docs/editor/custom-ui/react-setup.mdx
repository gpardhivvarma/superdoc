...
This page builds a hybrid interface: React renders the toolbar, and the remaining built-in surfaces stay at their defaults because the example passes no `ui`. [Who renders the UI?](/editor/who-renders-the-ui) owns the full comparison of built-in, hybrid, and custom modes.
