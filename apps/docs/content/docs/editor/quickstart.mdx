...
The editor above renders the document canvas and SuperDoc's default surfaces. The built-in toolbar needs its own mount target, while a hybrid or fully custom integration can move selected controls into application UI without changing how the DOCX is opened or saved.

[Who renders the UI?](/editor/who-renders-the-ui) compares all three ownership modes with a runnable example of each.
...
- [Configure the Editor](/editor/configuration) explains how `ui`, `interaction`, and `surfaces` divide up the configuration.
