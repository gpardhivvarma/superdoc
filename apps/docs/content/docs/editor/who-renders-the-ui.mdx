...
![The built-in UI and a custom application UI both control the same SuperDoc editor and Document API.](./_media/who-renders-the-ui/interface-paths.svg)
...
[Configure the Editor](/editor/configuration) explains how `ui`, `interaction`, and `surfaces` divide the configuration between them.
