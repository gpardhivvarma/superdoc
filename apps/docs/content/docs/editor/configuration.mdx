...
- `modules` configures integration subsystems such as tracked changes and AI.
- A structured document's `v2Collaboration` field configures its v2 collaboration room.
...
Rendering and permission are separate decisions, which is why `ui` and `interaction` are separate groups. [Who renders the UI?](/editor/who-renders-the-ui) owns that decision and compares the built-in, hybrid, and fully custom paths.
